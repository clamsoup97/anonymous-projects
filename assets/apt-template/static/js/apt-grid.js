(function(){
  ...
  function setupAPTGridDemo(root){
    console.log('[APT Demo] setupAPTGridDemo called with root:', root);
    const img = $('.apt-img', root);
    const gridCanvas = $('.apt-grid-canvas', root);
    console.log('[APT Demo] Found img:', img, 'canvas:', gridCanvas);
    if (!gridCanvas) {
      console.error('[APT Demo] No canvas found! Cannot initialize.');
      return;
    }
    const ctx = gridCanvas.getContext('2d');
    console.log('[APT Demo] Got canvas context:', ctx);
    ...
    function draw(){
      rafPending = false;
      console.log('[APT Demo] draw() called');
      setCanvasSize();
      const rect = img.getBoundingClientRect();
      const w = rect.width;
      const h = rect.height;
      console.log('[APT Demo] Canvas size:', w, 'x', h);
    ...
    const onImgLoad = function() {
      console.log('[APT Demo] Image loaded:', img.src, 'size:', img.naturalWidth, 'x', img.naturalHeight);
    ...
    console.log('[APT Demo] Image complete status:', img.complete);
    if (img.complete) {
      console.log('[APT Demo] Image already loaded, scheduling draw immediately');
      scheduleDraw();
    } else {
      console.log('[APT Demo] Waiting for image to load...');
    }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    console.log('[APT Demo] DOMContentLoaded event fired');
    const demos = document.querySelectorAll('.apt-grid-demo');
    console.log('[APT Demo] Found', demos.length, 'demo elements');
    demos.forEach((demo, i) => {
      console.log('[APT Demo] Initializing demo', i);
      try {
        setupAPTGridDemo(demo);
        console.log('[APT Demo] Successfully initialized demo', i);
      } catch (e) {
        console.error('[APT Demo] Error initializing demo', i, ':', e);
      }
    });
  });
})();
